...
def projection(P: np.ndarray, points_3d: np.ndarray) -> np.ndarray:
    ...
    if c != 4: 
        ...
        projected_points_2d = np.random.randn(r, 2)
...
def objective_func(x, **kwargs):
    ...
    P = np.reshape(x, (3, 4))
    projections = projection(P, p2).flatten()
    diff = -1*(projections - p1)
    #######################################################################
    #                           END OF YOUR CODE                          #
    #######################################################################
...
def estimate_camera_matrix(pts2d: np.ndarray,
    ...
    initial_guess = initial_guess[:-1].flatten()
    d = {"pts2d":pts2d,"pts3d":pts3d}
    diff = least_squares(objective_func, initial_guess, method='lm', verbose=2, max_nfev=5000, kwargs=d)
    M = diff.x
...
def calculate_camera_center(P: np.ndarray,
    ...
    # YOUR CODE HERE                                                      #
    #######################################################################
    cc = np.dot(np.linalg.inv(np.dot(-1*P[:, 0:3].T, -1*P[:, 0:3])), np.dot(-1*P[:, 0:3].T, P[:, 3]))
    #######################################################################
    #                           END OF YOUR CODE                          #
    #######################################################################
